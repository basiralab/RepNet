...
def extract_weights_single(dataset, view, model, training_type, shot_n, cv_n):
    ...
    if model == 'sag':
        weights_vector = torch.mean(weights['w'], 1).detach().cpu().numpy() 
    if model == 'diffpool':
...
def top_biomarkers(weights, K_i): 
    weights_normalized = np.abs(weights) # Calculate the absolute value element-wise.
    result = []
    w_sorted = weights_normalized.argsort()  
...
def view_specific_rep(dataset,view,training_type, models):
    ...
    for i in range(rep.shape[0]): 
        for j in range(rep.shape[1]): 
            weights_i = extract_weights(dataset, view, models[i], training_type) # 35'lik feature geri donuyor
...
def overall_avg_rep(data_dict):
    models = data_dict['models'] # ['sag']
    dataset = data_dict['dataset'] # 'LH_ASDNC'
    training_types = data_dict['training_types'] #  ['Few_Shot']
...
def GNN_specific_rep_accumulated_vect(dataset,views,training_type, model):
    ...
    rep_s = np.sum(rep,axis=1) # rep_s.shape: (4,4)
    rep_f = rep_s.flatten()
    rep_dict = {}
...
def manage_all_reps(data_dict):
    ...
    rep_dict['rank correlation'] = rep_corr['rank_matrix'] # r.c
...
data_dict={}
data_dict['dataset'] = 'LH_ASDNC' # 'LH_ADLMCI'
data_dict['views'] = [0, 1, 2, 3, 4, 5] #number of views 
data_dict['models'] = ['diffpool', 'gat', 'gcn', 'gunet', 'sag']
#data_dict['models'] = ['sag']
data_dict['training_types'] = ['5Fold'] #'Few_Shot'

Ks = [15]

rep_dict = manage_all_reps(data_dict)
name = data_dict['dataset'] + f"_Ks_{Ks[0]}_cv.pickle"
