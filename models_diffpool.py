...
class GraphConv(nn.Module):
    def __init__(self, input_dim, output_dim, add_self=False, normalize_embedding=False,
            dropout=0.0, bias=True, name='None'):
    ...
    def forward(self, x, adj):
        #print('GC epoch: ', self.current_epoch, self.name)
        if self.dropout > 0.001:
            x = self.dropout_layer(x)
        y = torch.matmul(adj, x)
        # Save weights of Diffpool Model
        if self.weight.shape[0]==35:
            if self.name != 'block':
                w_dict = {"w": self.weight}
                with open("Diffpool_W" + "_epoch"+str(self.current_epoch)+ ".pickle", 'wb') as f:
                    pickle.dump(w_dict, f) # weights
                    #    print("Diffpool Weights are saved.")
...
class GcnEncoderGraph(nn.Module):
    ...
    def build_conv_layers(self, input_dim, hidden_dim, embedding_dim, num_layers, add_self,
        ...
        conv_first = GraphConv(input_dim=input_dim, output_dim=hidden_dim, add_self=add_self,
                normalize_embedding=normalize, bias=self.bias, name='first')
        conv_block = nn.ModuleList(
                [GraphConv(input_dim=hidden_dim, output_dim=hidden_dim, add_self=add_self,
                        normalize_embedding=normalize, dropout=dropout, bias=self.bias, name='block') 
                 for i in range(num_layers-2)])
        conv_last = GraphConv(input_dim=hidden_dim, output_dim=embedding_dim, add_self=add_self,
                normalize_embedding=normalize, bias=self.bias, name='last')
    ...
    def loss(self, pred, label, type='softmax'):
        ...
        if type == 'softmax':
            ...
            label_onehot = torch.zeros(batch_size, self.label_dim).long().cpu()
...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        #asign_ratios = [assign_ratio1, assign_ratio2]
    ...
    def forward(self, x, adj, batch_num_nodes, **kwargs):
        ...
        epoch = kwargs['epoch']
        #print('epoch: ', epoch)
        self.conv_first_after_pool[0].current_epoch = epoch
        self.conv_block_after_pool[0].current_epoch = epoch
        self.conv_last_after_pool[0].current_epoch = epoch 
        self.assign_conv_first_modules[0].current_epoch = epoch
        self.assign_conv_block_modules[0].current_epoch = epoch
        self.assign_conv_last_modules[0].current_epoch = epoch
        self.conv_first.current_epoch = epoch
        self.conv_block.current_epoch = epoch
        self.conv_last.current_epoch = epoch
        ...
        if self.concat:
            ...
            output = out
        # output.shape: [1,2048]
        ypred = self.pred_model(output) # ypred.shape: [1,2]
        # abs
        ypred = torch.abs(ypred)
        return ypred
    ...
    def loss(self, pred, label, adj=None, batch_num_nodes=None, adj_hop=1):
        ...
        if self.linkpred:
            ...
            pred_adj = torch.min(pred_adj, torch.ones(1, dtype=pred_adj.dtype).cpu())
